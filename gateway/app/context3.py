...
# ---- Raw-text tracing --------------------------------------------------------
# One span, emitted only when TRACES_DATASET=1. Its input is the raw extracted
# page text - the actual input to passage scoring, including the passages that
# lose. tool_content is post-selection, so this is the only place they exist.
TRACES_DATASET = os.environ.get("TRACES_DATASET", "0") == "1"
TRACE_MAX_CHARS_PER_DOC = 12000   # Langfuse truncates ~1MB events, dropping fields silently


# Phrases that are unambiguously about the past. Deliberately narrow: broader
# substrings like "during the" or "the first person to" fire on plainly current
# questions ("pollen count during the summer", "first person to run a sub-2-hour
# marathon"). Only consulted when the model declares nothing.
_HISTORICAL_PHRASES = (
...
def _augment_time_sensitive_query(
    ...
    Two things differ from the original version:

    1. Whether to augment is decided by the resolved recency mode - which the
       model declares - not by scanning the query for recency words. A model
       composing a query frequently drops the recency wording it was answering
       ("most recent" -> "2025"), which made the query-only check miss exactly
       the cases that needed rewriting most.
    2. A year already in the query is only respected if it is current or future.
       A PAST year under recency intent is not a user-supplied constraint, it is
       the model's stale prior leaking into retrieval, so it is replaced rather
       than deferred to.
    """
    if mode is None:
...
def _score_result(
    ...
    try:
        searx_score = max(0.0, float(result.get("searx_score", 0) or 0))
...
def _trace_passage_inputs(docs: list[dict], query: str, mode: str, evidence: dict) -> None:
    """
    Raw page text in, selection out. Best-effort: tracing must never fail a search.
    """
    if not (TRACES_DATASET and docs):
        return
        ...
        with get_client().start_as_current_span(
            ...
            input={
                ...
                "documents": [
                    {
                        "url": d["url"],
                        "prior": round(d.get("prior", 0.0), 4),
                        "text": (d.get("text") or "")[:TRACE_MAX_CHARS_PER_DOC],
                    }
                    for d in docs
                ],
            },
            ...
            span.update(output={
                url: {"best_score": w["best_score"], "text": w["text"]}
                for url, w in evidence.items()
            })
    except Exception as e:
        logger.warning("passage trace failed: %s", e)
...
async def scrape_pages(urls: list[str]) -> dict[str, Optional[str]]:
    ...
    (as the original per-URL loop did) runs them one after another. gather() is
...
async def search_and_scrape(
    ...
    search_results = await search_searxng(request, query, candidate_count, question, mode)
    ...
    _trace_passage_inputs(docs, effective_query, mode, evidence)
    ...
    for result in ranked_results:
        ...
        result["recency_mode"] = mode
        result["recency_mode_source"] = mode_source
        result["recency_mode_heuristic"] = heuristic_guess
...
def format_search_context(
    ...
    if top_n is not None:
        results = sorted(
            results,
            key=lambda r: (
                r.get("final_score", r.get("retrieval_score", 0)),
                r.get("retrieval_score", 0),
            ),
            reverse=True,
        )[:top_n]
